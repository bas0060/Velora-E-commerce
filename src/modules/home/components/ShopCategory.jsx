...
const ShopCategory = () => {
  ...
  const categoryList = categories?.documents || [];

  const selectedCategory = categoryList.find(function (cat) {
  ...
  return (
    <div className="bg-[#F8F8F8] py-10">
      <div className="w-[90%] m-auto flex flex-col gap-y-6">
        <div className="flex flex-col gap-y-4 lg:flex-row items-start gap-x-20 justify-between">
          <div className="flex flex-col gap-y-4 md:gap-y-8 md:w-full lg:w-auto">
            <div className="flex flex-col gap-y-5">
              ...
              <h2 className="font-medium text-2xl md:text-3xl lg:text-5xl lg:leading-6 text-[#1A1A1A]">
                Shop by Category
              </h2>
            </div>

            {/* Buttons update selectedCategoryId */}
            <ShopCategoryButtons
          ...
          <div className="flex flex-col gap-y-6 md:flex-row items-start w-full lg:w-[63%] bg-[#A1C249] py-3 px-3 md:px-8 lg:pt-5 md:pb-10 rounded-xl lg:rounded-3xl">
            <div className="flex flex-col md:gap-y-8 w-full md:pt-16">
              <div className="flex flex-col gap-y-4 md:gap-y-2">
                <p className="text-[1.25rem] md:text-[1.45rem] font-medium text-[#1A1A1A] md:leading-10">
                  Limited Offer
                </p>
              ...
              <Link
                to={"/favorites"}
                className="hidden md:block bg-black w-[60%] md:w-[80%] lg:w-[60%]  rounded-[3rem]"
              >
                <div className="flex items-center justify-center gap-x-2 md:gap-x-4 py-2 md:py-4">
                  <p className="text-[1.2rem] md:text-[1.4rem] font-medium text-white md:leading-10">
                    Shop now
                  </p>
                  <div className=" bg-white rounded-full">
                    <img
                      src={arrowUp}
                      alt="arrowUp"
                      className="w-7 h-7 md:w-9 md:h-auto p-2"
                    />
                  </div>
                </div>
              </Link>
            ...
            <div className="flex md:hidden items-center flex-col gap-y-8">
              ...
              <Link
                to={"/favorites"}
                className="block md:hidden bg-black w-[60%] md:w-[60%] rounded-[3rem]"
              >
                <div className="flex items-center justify-center gap-x-2 md:gap-x-4 py-2 md:py-4">
                  <p className="text-[1.2rem] md:text-[1.4rem] font-medium text-white md:leading-10">
                    Shop now
                  ...
                  <div className=" bg-white rounded-full">
                    <img
                      src={arrowUp}
                      alt="arrowUp"
                      className="w-7 h-7 md:w-9 md:h-9 p-2 "
                    />
                  </div>
                </div>
              </Link>
        ...
        {/* New arrivals for selected category */}
        <ShopNewArrival
          categoryId={selectedCategoryId}
          categoryName={selectedCategory?.name || "All"}
        />
